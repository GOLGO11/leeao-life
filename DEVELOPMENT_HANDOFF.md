...
data/timeline-events-sixth-book.json              126
总计                                             893
```
...
```text
事件总数：893
日期精度统计：day=681；year=43；undated-range=1；undated-context=99；season=17；lunar-day=1；year-after=6；month=19；half-year=2；day-range=9；month-after=2；year-range=4；year-about=5；day-after=3；year-end=1
```
...
1. `134-142` 已从两份起诉状推进到 `137` 与 `141` 两份已校出确日的公报摘录；接下来应继续啃 `136`、`138`、`139`、`140`、`142`，但仍要先解决“公报期数对应绝对日期”的锚点问题，再落日级事件。
2. 当前暂缓的 `136-142` 内容已初步摸过：`136` 是审计长苏振平与国安密帐线，`138` 是苏贞昌与“终统/废统”“台独真假”线，`139` 是苏贞昌、台独公投、鞋袭混乱线，`140` 是程序发言权被剥夺线，`142` 是林庆隆被提名审计长质询线；`137` 与 `141` 已完成入库，不必重复做。
3. `121.刘忆如洒脱之至.txt`、`122.赵良燕现象.txt`、`125-131` 多为无确日评论、人物侧写、政党纲领或回忆，后续可在模糊项轮次处理。
4. `108`、`112`、`114-120`、`095` 等早前暂缓项，仍属模糊项候选。
