...
data/timeline-events-sixth-book.json              78
总计                                             845
```
...
```text
事件总数：845
日期精度统计：day=633；year=43；undated-range=1；undated-context=99；season=17；lunar-day=1；year-after=6；month=19；half-year=2；day-range=9；month-after=2；year-range=4；year-about=5；day-after=3；year-end=1
```
...
1. 可继续处理 `082-100`，因为 `081.喷瓦斯事件.txt` 已在第二轮处理过；建议先扫日期锚点，再按李敖本人动作拆分。
2. 本轮暂缓的 `063.最简明的智慧十题.txt`、`071.我的国民党观.txt`、`072.我的民进党观.txt`、`074.半坐半立式民主.txt` 偏无确日/评论总论；后续可在第六本模糊项轮次补，并放在对应年份精确日级事件之后。
3. `046.周公何辜啊！.txt` 暂未单独入库：它是李敖对杜正胜睡觉、挖鼻孔、本土派转向的评论短文，缺精确日期；后续可在第六本模糊项轮次补。
4. `036.国家安全局对我下毒？.txt` 暂未单独入库：它含送香蕉、退国安局礼金、国安局视察时换便当等李敖日常动作，但日期不精确；后续可在第六本模糊项轮次集中补。
5. `031.我炮口转向了.txt` 暂未入库：它是李敖转向诉讼/外部攻击的策略说明，缺精确日期；后续可在 2005 年模糊项集中处理时加入。
6. 再分批处理 `134-142` 的起诉状与立法院公报，按“每轮只吃几份”原则拆成法律/院会链，不要一口气吞大文件。
7. 继续时要注意：Book 6 原始 txt 多为 GB18030，直接 Read 常出现乱码；可靠流程是先 `iconv -f GB18030 -t UTF-8 ... | nl -ba`，再据此写 source.line。
...
1. 优先处理 `082-100`，注意 `081.喷瓦斯事件.txt` 已处理过，避免重复。
2. 回头集中处理 `031.我炮口转向了.txt`、`036.国家安全局对我下毒？.txt`、`046.周公何辜啊！.txt`、`063.最简明的智慧十题.txt`、`071.我的国民党观.txt`、`072.我的民进党观.txt`、`074.半坐半立式民主.txt` 这类缺确日但能说明李敖策略/日常/评论动作的模糊项，确保放在对应年份精确日级事件之后。
