...
data/timeline-events-sixth-book.json              121
总计                                             888
```
...
```text
事件总数：888
日期精度统计：day=676；year=43；undated-range=1；undated-context=99；season=17；lunar-day=1；year-after=6；month=19；half-year=2；day-range=9；month-after=2；year-range=4；year-about=5；day-after=3；year-end=1
```
...
1. 可继续处理 `125-131` 与 `134-142` 之间的衔接地带。`124` 已确认为建党日主锚点，但 `125-127` 多属中国智慧党宣言/章程/测量表，可按需要在模糊项或思想项轮次补。
2. 本轮暂缓的 `121.刘忆如洒脱之至.txt`、`122.赵良燕现象.txt`、`125.“中国智慧党”灌顶宣言.txt`、`126.“中国智慧党”章程.txt`、`127.“中国智慧党”智慧测量表.txt`、`128.我的台湾人朋友.txt`、`129.伪国会.txt`、`130.蒋介石阴魂不散.txt`、`131.“桃花圣解”十五则.txt` 多为无确日评论、人物侧写、政党纲领或回忆，后续可在模糊项轮次处理。
3. 早前暂缓的 `108.两组耐人寻味的名单.txt`、`112.“他妈的”与“你妈的”.txt`、`114.张显耀值得特别表扬.txt`、`115-120` 也仍属模糊项候选。
4. `095.李杰下台了！.txt` 仍未入库：有记者会、送大奶嘴和给李杰信，但章节未给精确日期；后续可结合外部或后续章节校日后再入库。
...
1. 优先处理 `134-142`，但务必分批拆，不要一次性吞下整个附录区。
